...
func toPebbleCmd() *cobra.Command {
	...
	cmd.Flags().String("sync-id", "", "Source sync ID to convert. Default: latest finished full sync (baton reads full-sync snapshots). Empty c1z if the source has no syncs.")
...
func runToPebble(cmd *cobra.Command, args []string) error {
	...
	convertOpts := []dotc1z.ConvertOption{
		dotc1z.WithConvertBatchSize(batchSize),
