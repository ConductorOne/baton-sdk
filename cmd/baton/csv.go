...
func fetchResourceTypes(ctx context.Context, store *dotc1z.C1File) (map[string]*v2.ResourceType, error) {
...
func fetchResources(ctx context.Context, store *dotc1z.C1File) (map[string]*v2.Resource, map[string]map[string]*v2.Resource, error) {
...
func fetchEntitlements(ctx context.Context, store *dotc1z.C1File) (map[string]*v2.Entitlement, map[string]map[string]*v2.Entitlement, error) {
...
func fetchGrants(ctx context.Context, store *dotc1z.C1File) (map[string]*v2.Grant, map[string]map[string]*v2.Grant, error) {
...
func buildCSV(ctx context.Context, d dataBag, outPath string) error {
	...
	for _, userType := range userTypes {
		...
		for _, r := range rs {
			ut := &v2.UserTrait{}
			ok, err = prototools.UnmarshalFromAnys(ut, r.Annotations)
			if err != nil {
				return err
			}
	...
	for _, g := range d.grantsByID {
		if p, ok := d.resourcesByID[fmtResourceID(g.Principal.Id)]; ok {
			...
			ok, err = prototools.UnmarshalFromAnys(ut, p.Annotations)
			if err != nil {
				return err
			}
