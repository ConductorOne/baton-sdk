...
func (c *bulkCreateTicketTaskHandler) HandleTask(ctx context.Context) error {
	...
	for _, createTicketTask := range t.GetTicketRequests() {
		requestBuilder := &v2.TicketsServiceCreateTicketRequest_builder{
		...
		ticketRequests = append(ticketRequests, requestBuilder.Build())
	}

	cc := c.helpers.ConnectorClient()
	requestBuilder := &v2.TicketsServiceBulkCreateTicketsRequest_builder{
		TicketRequests: ticketRequests,
	}
	resp, err := cc.BulkCreateTickets(ctx, requestBuilder.Build())
	if err != nil {
