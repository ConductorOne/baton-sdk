...
func TestMemorySessionCache_GetMany(t *testing.T) {
	t.Run("successful getMany", func(t *testing.T) {
		...
		// First call
		values, pageToken, err := cache.GetMany(ctx, []string{"key1", "key2"}, sessions.WithSyncID("sync-1"))
		require.NoError(t, err)
	...
	t.Run("getMany with prefix", func(t *testing.T) {
		...
		values, pageToken, err := cache.GetMany(ctx, []string{"key1", "key2"}, sessions.WithSyncID("sync-1"), sessions.WithPrefix("prefix/"))
		require.NoError(t, err)
		require.Equal(t, "", pageToken)
		require.Equal(t, 2, len(values))
	...
	t.Run("cross-contamination with GetMany", func(t *testing.T) {
		...
		backingStoreCallCount = 0
		values, pageToken, err = cache.GetMany(ctx, []string{"key1", "key2"}, sessions.WithSyncID("sync-2"))
		require.NoError(t, err)
		require.Equal(t, "", pageToken)
		require.Equal(t, 2, len(values))
