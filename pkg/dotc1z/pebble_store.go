...
func (s *pebbleStore) CloseEngineOnly() error {
	...
	s.closeMu.Lock()
	if s.admission == pebbleStoreClosed {
		s.closeMu.Unlock()
		return nil
	}
...
// admission as the direct engine mutations that produced it.
...
func (s *pebbleStore) Close(ctx context.Context) error {
	...
	if !s.readOnly && dirty {
		if err := s.save(ctx); err != nil {
			retErr = fmt.Errorf("pebble store close: save failed, store left open and unsaved data preserved under %s: %w", s.tmpDir, err)
			s.closeMu.Lock()
			s.admission = pebbleStoreOpen
			s.lastCloseErr = retErr
			s.cond().Broadcast()
			s.closeMu.Unlock()
			return retErr
		}
