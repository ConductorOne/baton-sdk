...
// modelSmokeSeeds is how many fixed seeds the ordinary-CI smoke leg
// runs; the nightly sweep starts where the smoke leg stops so the two
// never duplicate work.
const modelSmokeSeeds = 3

// TestModelRandomizedSourceCacheLifecycleSmoke is the ordinary-CI leg
// of the randomized model oracle: a few fixed seeds (~1s each) so a
// lifecycle regression in this silent+durable subsystem fails the
// offending PR instead of surfacing in the next nightly. Skipped under
// -short (the Windows CI leg), like the rest of the heavier store
// tests.
func TestModelRandomizedSourceCacheLifecycleSmoke(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping model smoke in short mode")
	}
	for seed := range modelSmokeSeeds {
