...
func TestReadHeaderFormat(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr != nil {
				require.ErrorIs(t, err, tc.wantErr, "err: got %v, want %v", err, tc.wantErr)
			} else {
				require.NoError(t, err, "unexpected err: %v", err)
			}
			require.Equal(t, tc.want, got, "format: got %v, want %v", got, tc.want)
		})
	}
}
...
func TestReadHeaderFormat_SeekerRewinds(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, C1ZFormatV3, got, "got %v want v3", got)
}
...
func TestFormatString(t *testing.T) {
	for _, tc := range []struct {
		...
		require.Equal(t, tc.want, tc.f.String(), "String(%d): got %q want %q", tc.f, tc.f.String(), tc.want)
	}
}
...
func TestErrEngineNotAvailable(t *testing.T) {
	...
	require.NotNil(t, ErrEngineNotAvailable, "ErrEngineNotAvailable must be set")
	wrapped := errors.New("upstream: " + ErrEngineNotAvailable.Error())
	require.Contains(t, wrapped.Error(), "engine not available", "error message changed unexpectedly: %v", wrapped)
}
