...
// listGrantsPayloadInternal returned grant payloads only (the dead
// GrantListModePayload path). The exported listing API no longer offers
// a payload-only mode, but the helper is retained for the c1ztest
// package until we validate it has no remaining users. If confirmed
// dead, delete in a follow-up.
func (c *C1File) listGrantsPayloadInternal(ctx context.Context, opts grantListOptions) (*internalGrantListResponse, error) {
	if err := c.validateDb(ctx); err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		result = append(result, &internalGrantRow{Grant: grant})
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
