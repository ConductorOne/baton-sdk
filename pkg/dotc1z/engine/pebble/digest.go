...
//
//	index key   = partitionPrefix(sync, partition) | <raw bucket hash: digestBucketHashLen bytes> | <anything>
//	index value = content hash (hashLen bytes)
//
// and, for partition enumeration,
//
//	index key   = syncBounds(sync).lower | 0x00 | tuple(partition) | 0x00 | <bucket hash> | …
//
// (i.e. the partition is the first tuple component after the sync-wide
// lower bound — the standard by-value index layout).
//
...
type digestIndexSpec struct {
	...
	partitionPrefix func(syncIDBytes []byte, partition string) []byte

	// syncBounds bounds the index's entire keyspace for one sync.
	syncBounds func(syncIDBytes []byte) (lower, upper []byte)
}
...
func (s digestIndexSpec) bucketBounds(idBytes []byte, partition string, b DigestBucket) ([]byte, []byte) {
	prefix := s.partitionPrefix(idBytes, partition)
	if b.Bits == 0 {
...
func (e *Engine) buildPartitionDigest(ctx context.Context, spec digestIndexSpec, idBytes []byte, partition string) error {
	prefix := spec.partitionPrefix(idBytes, partition)
	count, err := e.countKeysInRange(prefix, upperBoundOf(prefix))
	if err != nil {
		return err
	}
	return e.buildPartitionDigestAtWidth(ctx, spec, idBytes, partition, chooseDigestWidth(count))
}
...
func (e *Engine) buildPartitionDigestAtWidth(ctx context.Context, spec digestIndexSpec, idBytes []byte, partition string, widthBits int) error {
	prefix := spec.partitionPrefix(idBytes, partition)
	upper := upperBoundOf(prefix)
	nodeLower := encodeDigestPartitionPrefix(idBytes, spec.indexID, partition)
	...
	return e.withWrite(func() error {
		...
		flushLeaf := func() error {
			...
			key := encodeDigestNodeKey(idBytes, spec.indexID, partition, digestLevelLeaf, lp[:])
		...
		rootKey := encodeDigestNodeKey(idBytes, spec.indexID, partition, digestLevelRoot, nil)
...
// distinctDigestPartitions returns the distinct partitions present in a
// sync's digested index, in index order. It seeks past each partition's
// whole range once its id is captured, so the cost is O(partitions)
// seeks, not O(entries). This is the diff driver's work list, and it
// deliberately comes from the INDEX rather than the partition-owning
// records: an entry whose partition has no owning record still has
// index entries (and a fold), so it still gets compared — only a digest
// was never built for it.
func (e *Engine) distinctDigestPartitions(ctx context.Context, spec digestIndexSpec, syncIDBytes []byte) ([]string, error) {
	lower, upper := spec.syncBounds(syncIDBytes)
	iter, err := e.db.NewIter(&pebble.IterOptions{LowerBound: lower, UpperBound: upper})
	if err != nil {
		return nil, err
	}
	...
	// Keys are lower | 0x00 | tuple(partition) | 0x00 | bucketHash | …
	// (the spec's shape contract).
	partStart := len(lower) + 1
	var out []string
	for iter.First(); iter.Valid(); {
		...
		key := iter.Key()
		if len(key) <= partStart {
			iter.Next()
			continue // malformed; skip defensively
		}
		partBytes, _, decErr := codec.DecodeTupleStringTo(nil, key[partStart:], 0)
		if decErr != nil {
			iter.Next()
			continue
		}
		partition := string(partBytes)
		out = append(out, partition)
		// Seek past this partition's whole index range.
		seekTo := upperBoundOf(spec.partitionPrefix(syncIDBytes, partition))
		if seekTo == nil {
			break
		}
		iter.SeekGE(seekTo)
...
// on demand).
func (e *Engine) getPartitionDigestRoot(spec digestIndexSpec, idBytes []byte, partition string) (DigestRoot, bool, error) {
	val, closer, err := e.db.Get(encodeDigestNodeKey(idBytes, spec.indexID, partition, digestLevelRoot, nil))
	if err != nil {
...
func (e *Engine) getDigestLeaf(spec digestIndexSpec, idBytes []byte, partition string, leafPrefix []byte) (int64, []byte, bool, error) {
	val, closer, err := e.db.Get(encodeDigestNodeKey(idBytes, spec.indexID, partition, digestLevelLeaf, leafPrefix))
	if err != nil {
		if errors.Is(err, pebble.ErrNotFound) {
...
// split-independent.
func (e *Engine) foldedLeafBuckets(ctx context.Context, spec digestIndexSpec, idBytes []byte, partition string, foldBits int) ([]foldedBucket, error) {
	stem := encodeDigestNodeKey(idBytes, spec.indexID, partition, digestLevelLeaf, nil)
...
func (e *Engine) computeBucketDigest(ctx context.Context, spec digestIndexSpec, idBytes []byte, partition string, bucket DigestBucket) ([]byte, int64, error) {
	lower, upper := spec.bucketBounds(idBytes, partition, bucket)
	iter, err := e.db.NewIter(&pebble.IterOptions{LowerBound: lower, UpperBound: upper})
	if err != nil {
		return nil, 0, err
	}
...
func (e *Engine) dirtyPartitionBuckets(ctx context.Context, spec digestIndexSpec, idBytes []byte, other *Engine, otherIDBytes []byte, partition string) ([]DigestBucket, error) {
	rootA, okA, err := e.getPartitionDigestRoot(spec, idBytes, partition)
	if err != nil {
		return nil, err
	}
	rootB, okB, err := other.getPartitionDigestRoot(spec, otherIDBytes, partition)
	if err != nil {
		return nil, err
	}

	if !okA || !okB {
		ha, ca, err := e.computeBucketDigest(ctx, spec, idBytes, partition, DigestBucket{})
		if err != nil {
			return nil, err
		}
		hb, cb, err := other.computeBucketDigest(ctx, spec, otherIDBytes, partition, DigestBucket{})
		if err != nil {
			return nil, err
		}
	...
	fa, err := e.foldedLeafBuckets(ctx, spec, idBytes, partition, compareBits)
	if err != nil {
		return nil, err
	}
	fb, err := other.foldedLeafBuckets(ctx, spec, otherIDBytes, partition, compareBits)
	if err != nil {
		return nil, err
	}
...
func (m *digestMutator) partFor(idBytes []byte, partition string) (*mutatorPartition, error) {
	rootKey := encodeDigestNodeKey(idBytes, m.spec.indexID, partition, digestLevelRoot, nil)
	k := string(rootKey)
	if mp, ok := m.parts[k]; ok {
		return mp, nil
	}
	mp := &mutatorPartition{idBytes: idBytes, partition: partition, rootKey: rootKey, nodes: make(map[string]*mutatorNode)}
...
func (m *digestMutator) addHash(idBytes []byte, partition string, bucketHash, contentHash []byte) error {
	return m.delta(idBytes, partition, bucketHash, contentHash, 1)
}

// removeHash records the removal of a record from its partition's
// digest.
func (m *digestMutator) removeHash(idBytes []byte, partition string, bucketHash, contentHash []byte) error {
	return m.delta(idBytes, partition, bucketHash, contentHash, -1)
}

func (m *digestMutator) delta(idBytes []byte, partition string, bucketHash, contentHash []byte, sign int64) error {
	mp, err := m.partFor(idBytes, partition)
	if err != nil {
		return err
	}
	if !mp.present {
	...
	key := encodeDigestNodeKey(idBytes, m.spec.indexID, partition, digestLevelLeaf, bucketOfHash(bucketHash, mp.bits).leafKeyPrefix())
	nk := string(key)
	n, ok := mp.nodes[nk]
	if !ok {
...
func (m *digestMutator) applyPartition(batch *pebble.Batch, mp *mutatorPartition) error {
	...
	dropDigest := func() error {
		...
		lo := encodeDigestPartitionPrefix(mp.idBytes, m.spec.indexID, mp.partition)
