...
func TestSourceCacheReplayGrantsAcrossEngines(t *testing.T) {
	...
	sealReplaySource(ctx, t, prev.PebbleEngine(), sourcecache.RowKindGrants, scopeA)
	...
	// current store (as next sync's previous) finds the same rows. The
	// manifest entry does NOT come across — writing it (with the unchanged
	// validator) is orchestration's job, mirrored here before sealing.
	sealReplaySource(ctx, t, cur.PebbleEngine(), sourcecache.RowKindGrants, scopeA)
	next := newAdapter(t)
...
func TestSourceCacheReplayRejectsStaleIndexEntries(t *testing.T) {
	...
	// Seal BEFORE the corruption below: scopeA's sealed count is an honest
	// zero, so the stale index is detected by the walk, not the count.
	sealReplaySource(ctx, t, prev.PebbleEngine(), sourcecache.RowKindGrants, scopeA, scopeB)
	...
	require.NoError(t, prev.PebbleEngine().UnsafeForTesting().Set(staleIdx, nil, nil))
	...
	// The genuine scope still replays.
...
// entry (asserted via preflight, which hard-errors on a stale index) —
// and must POISON the old scope (CO-015): its manifest entry vouches for
// a row the restamp just took, so replaying it would silently drop the
// row. The new scope owns the rewritten row exactly.
func TestEntitlementRestampCleansOldScopeIndex(t *testing.T) {
	...
	sealReplaySource(ctx, t, prev.PebbleEngine(), sourcecache.RowKindEntitlements, scopeA, scopeB)

	cur := newAdapter(t)
	_, err = cur.StartNewSync(ctx, connectorstore.SyncTypeFull, "")
	require.NoError(t, err)

	// Old scope: poisoned by the restamp — refused as a replay source.
	// (The index cleanup itself is proven by scopeB's clean preflight
	// below: a stale scopeA entry would fail it with "resolves to
	// primary stamped".)
...
func TestGrantAndResourceRestampCleanOldScopeIndexes(t *testing.T) {
	ctx := t.Context()
	prev := newAdapter(t)
	_, err := prev.StartNewSync(ctx, connectorstore.SyncTypeFull, "")
	require.NoError(t, err)
	...
	require.NoError(t, prev.PutGrants(sourcecache.WithScope(ctx, scopeA), grant))
	require.NoError(t, prev.PutGrants(sourcecache.WithScope(ctx, scopeB), grant))

	resource := v2.Resource_builder{
		Id:          v2.ResourceId_builder{ResourceType: "user", Resource: "alice"}.Build(),
		DisplayName: "Alice",
	}.Build()
	require.NoError(t, prev.PutResources(sourcecache.WithScope(ctx, scopeA), resource))
	resource.SetDisplayName("Alice Updated")
	require.NoError(t, prev.PutResources(sourcecache.WithScope(ctx, scopeB), resource))
	sealReplaySourceMulti(ctx, t, prev.PebbleEngine(), map[sourcecache.RowKind][]string{
		sourcecache.RowKindGrants:    {scopeA, scopeB},
		sourcecache.RowKindResources: {scopeA, scopeB},
	})
	...
	// Old scopes: poisoned by the restamps (CO-015) — refused as replay
	// sources rather than replaying silently-empty sets. New scopes own
	// the rows exactly; their clean preflights prove the old index
	// entries were removed.
	_, err = cur.PebbleEngine().ReplaySourceCacheGrants(ctx, prev.PebbleEngine(), scopeA)
	require.ErrorContains(t, err, "poisoned")
	newGrant, err := cur.PebbleEngine().ReplaySourceCacheGrants(ctx, prev.PebbleEngine(), scopeB)
	require.NoError(t, err)
	require.Equal(t, int64(1), newGrant.Rows)

	_, err = cur.PebbleEngine().ReplaySourceCacheResources(ctx, prev.PebbleEngine(), scopeA)
	require.ErrorContains(t, err, "poisoned")
	newResource, err := cur.PebbleEngine().ReplaySourceCacheResources(ctx, prev.PebbleEngine(), scopeB)
	require.NoError(t, err)
	require.Equal(t, int64(1), newResource.Rows)
}
...
func TestDeleteGrantsByPrincipalsInScope(t *testing.T) {
	...
	sealReplaySource(ctx, t, a.PebbleEngine(), sourcecache.RowKindGrants, scopeA)
	cur := newAdapter(t)
	_, err = cur.StartNewSync(ctx, connectorstore.SyncTypeFull, "")
	require.NoError(t, err)
...
func TestSourceCacheReplayResourcesAndEntitlements(t *testing.T) {
	...
	sealReplaySourceMulti(ctx, t, prev.PebbleEngine(), map[sourcecache.RowKind][]string{
		sourcecache.RowKindResources:    {scopeA},
		sourcecache.RowKindEntitlements: {scopeA},
	})
...
func TestReplayInvalidatesEntitlementIDLookup(t *testing.T) {
	...
	ent1 := v2.Entitlement_builder{Id: entID, Resource: res1, DisplayName: "member"}.Build()
	require.NoError(t, prev.PutEntitlements(sourcecache.WithScope(ctx, scopeA), ent1))
	sealReplaySource(ctx, t, prev.PebbleEngine(), sourcecache.RowKindEntitlements, scopeA)
