...
func TestGrantAndResourceRestampCleanOldScopeIndexes(t *testing.T) {
	...
	oldGrant, err := cur.PebbleEngine().ReplaySourceCacheGrants(ctx, prev.PebbleEngine(), scopeA)
	require.NoError(t, err)
	require.Zero(t, oldGrant.Rows)
	require.Zero(t, oldGrant.StaleSkipped)
	newGrant, err := cur.PebbleEngine().ReplaySourceCacheGrants(ctx, prev.PebbleEngine(), scopeB)
	require.NoError(t, err)
	require.Equal(t, int64(1), newGrant.Rows)

	oldResource, err := cur.PebbleEngine().ReplaySourceCacheResources(ctx, prev.PebbleEngine(), scopeA)
	require.NoError(t, err)
	require.Zero(t, oldResource.Rows)
	require.Zero(t, oldResource.StaleSkipped)
