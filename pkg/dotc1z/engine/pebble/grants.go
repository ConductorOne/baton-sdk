	...
	return e.withWrite(func() error {
		...
		for i, r := range records {
			...
			case getErr == nil:
				...
				closer.Close()
			case errors.Is(getErr, pebble.ErrNotFound):
				// No prior record: stamping handled by the fallback below.
			default:
				return fmt.Errorf("PutExpandedGrantRecords: get old %q: %w", ext, getErr)
			}

			// Stamp now for a genuinely new record, or for a prior
			// that somehow lacked a timestamp (the preserved
			// discovered_at above can be nil). Matches the old
			// StoreExpandedGrants fallback so a degenerate nil prior
			// isn't left nil.
		...
		// NoSync unconditionally — including on resumed (non-fresh)
		// syncs, unlike PutGrantRecords which only downgrades to NoSync
		// for fresh syncs. Expanded grants are fully regenerable from
		// the sync (the expander recomputes them from the entitlement
		// graph), so a per-batch fsync buys nothing. These writes are
		// hardened by the single Flush at sync end (EndFreshSync) or by
		// the unconditional Flush in Engine.Close/Quiesce, both of which
		// run before the c1z is saved — so durability is preserved even
		// without gating on IsFreshSync(). See the method doc.
