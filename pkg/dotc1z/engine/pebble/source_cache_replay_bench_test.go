...
func BenchmarkSourceCacheReplayResources(b *testing.B) {
	for _, rows := range []int{1_000, 10_000, 100_000} {
		b.Run(fmt.Sprintf("rows-%d", rows), func(b *testing.B) {
			...
			// Replay preflight (CO-014) demands a manifest entry sealed by
			// a counting EndSync; both happen once, outside the timed loop.
			if err := prev.PutSourceCacheEntry(ctx, "resources", "benchmark-scope", "bench-validator"); err != nil {
				b.Fatal(err)
			}
			if err := prev.EndSync(ctx); err != nil {
				b.Fatal(err)
			}
