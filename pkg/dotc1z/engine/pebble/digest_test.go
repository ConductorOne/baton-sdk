...
func digestNodeCount(t testing.TB, e *Engine, syncID string) int {
	...
	iter, err := e.db.NewIter(&pebble.IterOptions{
		LowerBound: DigestSyncLowerBound(idBytes),
		UpperBound: DigestSyncUpperBound(idBytes),
	})
	if err != nil {
...
func rawLeafPrefixes(t testing.TB, e *Engine, idBytes []byte, entID string) [][]byte {
	t.Helper()
	stem := encodeDigestNodeKey(idBytes, grantDigestSpec.indexID, entID, digestLevelLeaf, nil)
...
func seedEntitlementAtWidth(t testing.TB, e *Engine, entID string, grants []*v3.GrantRecord, widthBits int) string {
	...
	if err := e.buildPartitionDigestAtWidth(ctx, grantDigestSpec, idBytes, entID, widthBits); err != nil {
...
func TestDigestDifferentWidthsComparison(t *testing.T) {
	...
	if err := eb.buildPartitionDigestAtWidth(ctx, grantDigestSpec, idBytesB, "ent-A", 8); err != nil {
...
func TestDigestLeafFoldConsistent(t *testing.T) {
	...
	leaves, err := e.foldedLeafBuckets(ctx, grantDigestSpec, idBytes, "ent-A", 8)
	if err != nil {
		t.Fatal(err)
	}
	if len(leaves) == 0 {
	...
	// build-width leaves.
	leaves4, err := e.foldedLeafBuckets(ctx, grantDigestSpec, idBytes, "ent-A", 4)
	if err != nil {
		t.Fatal(err)
	}
	...
	lc, ld, present, err := e.getDigestLeaf(grantDigestSpec, idBytes, "ent-A", b.leafKeyPrefix())
	if err != nil || !present {
...
func TestDigestRebuildClearsStaleNodes(t *testing.T) {
	...
	before := rawLeafPrefixes(t, e, idBytes, "ent-A")
	if len(before) == 0 {
	...
	if err := e.buildPartitionDigestAtWidth(ctx, grantDigestSpec, idBytes, "ent-A", 4); err != nil {
	...
	after := rawLeafPrefixes(t, e, idBytes, "ent-A")
...
func TestDigestIncrementalEqualsRebuild(t *testing.T) {
	...
	if !shared {
		_, _, present, err := e.getDigestLeaf(grantDigestSpec, idBytes, "ent-A", deletedLeaf)
		if err != nil {
			t.Fatal(err)
		}
		if present {
	...
	if err := e.buildPartitionDigestAtWidth(ctx, grantDigestSpec, idBytes, "ent-A", 8); err != nil {
...
func TestDigestSameBatchSameBucketRMW(t *testing.T) {
	...
	count, _, present, err := e.getDigestLeaf(grantDigestSpec, idBytes, "ent-A", []byte{target, 0})
	if err != nil || !present {
