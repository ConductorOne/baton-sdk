...
// in grant_digest.go): digest nodes present without a stamp naming the
...
func TestGrantDigestABIStaleStampDroppedAtWritableOpen(t *testing.T) {
	...
	setABIStamp(t, e, 1) // a fake old ABI version
...
// TestGrantDigestABIMissingStampTreatedAsStale is
// TestGrantDigestABIStaleStampDroppedAtWritableOpen but with the stamp
// key DELETED outright rather than rewritten — simulating a file
// sealed by a pre-stamp SDK build (digest nodes exist, no stamp at
// all). Absence-with-nodes-present must be treated exactly like a
// wrong-version stamp: dropped wholesale by a writable open.
func TestGrantDigestABIMissingStampTreatedAsStale(t *testing.T) {
	ctx := context.Background()
	const entID = "ent-A"
	const n = 20
	...
	require.Zero(t, digestNodeCount(t, e2), "missing-stamp writable open must drop every digest node")
	require.Zero(t, countKeyRangeTest(t, e2, GrantByEntPrincHashLowerBound(), GrantByEntPrincHashUpperBound()),
		"missing-stamp writable open must drop the whole hash index")
	_, ok, err := e2.GetGrantDigestGlobalRoot(ctx)
	require.NoError(t, err)
	require.False(t, ok, "global root must read as absent after the drop")
