...
// TestListResourcesTraitFilterPebble exercises the trait-filter
// post-filter on the Pebble adapter (RFC §B2).
func TestListResourcesTraitFilterPebble(t *testing.T) {
	ctx := context.Background()
	a := newAdapter(t)
	if _, err := a.StartNewSync(ctx, connectorstore.SyncTypeFull, ""); err != nil {
		t.Fatal(err)
	...
	if err := a.PutResources(ctx,
		...
		mkRes("contractor", "c1"),
		mkRes("team", "t1"), mkRes("team", "t2"),
	); err != nil {
		t.Fatal(err)
	}

	t.Run("trait=USER", func(t *testing.T) {
		...
		if got := len(resp.GetList()); got != 3 {
			t.Errorf("len=%d, want 3", got)
		}
		for _, r := range resp.GetList() {
			rt := r.GetId().GetResourceType()
			if rt != "employee" && rt != "contractor" {
				t.Errorf("unexpected rt %q", rt)
			}
		}
	})
	...
	t.Run("trait + non-matching RT returns empty", func(t *testing.T) {
		resp, err := a.ListResources(ctx, v2.ResourcesServiceListResourcesRequest_builder{
			Trait:          v2.ResourceType_TRAIT_USER,
			ResourceTypeId: "team",
			PageSize:       100,
