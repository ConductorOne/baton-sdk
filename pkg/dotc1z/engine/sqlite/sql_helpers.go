...
func (c *SQLite) throttledWarnSlowQuery(ctx context.Context, query string, duration time.Duration) {
...
func (c *SQLite) listConnectorObjects(ctx context.Context, tableName string, req proto.Message) ([][]byte, string, error) {
	ctx, span := tracer.Start(ctx, "SQLite.listConnectorObjects")
	defer span.End()
	...
	if !ok {
		return nil, "", fmt.Errorf("SQLite: invalid list request")
	}
	...
	default:
		var latestSyncRun *engine.SyncRun
		var err error
		latestSyncRun, err = c.getFinishedSync(ctx, 0, engine.SyncTypeFull)
		if err != nil {
			return nil, "", err
		}
...
func bulkPutConnectorObject[T proto.Message](
	...
	ctx, span := tracer.Start(ctx, "SQLite.bulkPutConnectorObject")
	defer span.End()
...
func bulkPutConnectorObjectIfNewer[T proto.Message](
	...
	ctx, span := tracer.Start(ctx, "SQLite.bulkPutConnectorObjectIfNewer")
	defer span.End()
...
func (c *SQLite) getResourceObject(ctx context.Context, resourceID *v2.ResourceId, m *v2.Resource, syncID string) error {
	ctx, span := tracer.Start(ctx, "SQLite.getResourceObject")
	defer span.End()
...
func (c *SQLite) getConnectorObject(ctx context.Context, tableName string, id string, syncID string, m proto.Message) error {
	ctx, span := tracer.Start(ctx, "SQLite.getConnectorObject")
	defer span.End()
	...
	default:
		var latestSyncRun *engine.SyncRun
		var err error
		latestSyncRun, err = c.getFinishedSync(ctx, 0, engine.SyncTypeAny)
		if err != nil {
