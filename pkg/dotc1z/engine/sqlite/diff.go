...
func (c *SQLite) GenerateSyncDiff(ctx context.Context, baseSyncID string, appliedSyncID string) (string, error) {
	...
	if err := c.insertSyncRun(ctx, diffSyncID, engine.SyncTypePartial, baseSyncID); err != nil {
		return "", err
	}
...
func (c *SQLite) diffTableQuery(table tableDescriptor, baseSyncID, appliedSyncID, newSyncID string) (string, []any, error) {
