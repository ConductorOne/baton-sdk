...
func (c *C1File) Stats(ctx context.Context, syncType connectorstore.SyncType, syncId string) (map[string]int64, error) {
	...
	resp, err := c.GetSync(ctx, requestBuilder.Build())
	if err != nil {
		return nil, err
	}
	if resp == nil || resp.GetSync() == nil {
	...
	pageToken := ""
	for {
		requestBuilder := &v2.ResourceTypesServiceListResourceTypesRequest_builder{
			PageToken: pageToken,
		}
		resp, err := c.ListResourceTypes(ctx, requestBuilder.Build())
		if err != nil {
			return nil, err
		}
...
func (c *C1File) GrantStats(ctx context.Context, syncType connectorstore.SyncType, syncId string) (map[string]int64, error) {
	...
	if syncId == "" {
		...
		lastSync, err := c.GetSync(ctx, requestBuilder.Build())
		if err != nil {
			return nil, err
		}
		if lastSync == nil {
