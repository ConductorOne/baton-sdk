...
func forEachSessionStoreEngine(t *testing.T, fn func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore)) {
	...
	for _, engine := range []c1zstore.Engine{c1zstore.EngineSQLite, c1zstore.EnginePebble} {
		t.Run(string(engine), func(t *testing.T) {
			...
			fn(t, ctx, store, syncID, store.SessionStore())
		})
	}
}
...
func TestSessionStore_Get(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		...
		t.Run("Get existing key", func(t *testing.T) {
			// Set a value first
			err := ss.Set(ctx, "test-key", []byte("test-value"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Get the value
			value, found, err := ss.Get(ctx, "test-key", sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("Get with prefix", func(t *testing.T) {
			...
			err := ss.Set(ctx, "prefixed-key", []byte("prefixed-value"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("test-prefix:"))
			require.NoError(t, err)

			// Get the value with prefix
			value, found, err := ss.Get(ctx, "prefixed-key",
				sessions.WithSyncID(syncID), sessions.WithPrefix("test-prefix:"))
			require.NoError(t, err)
			require.True(t, found)
		...
		t.Run("Get without sync ID", func(t *testing.T) {
			_, _, err := ss.Get(ctx, "test-key")
			require.Error(t, err)
			require.Contains(t, err.Error(), "sync id is required")
		})

		t.Run("Get with wrong sync ID", func(t *testing.T) {
			value, found, err := ss.Get(ctx, "test-key", sessions.WithSyncID("wrong-sync-id"))
			require.NoError(t, err)
			require.False(t, found)
			require.Nil(t, value)
		})
...
func TestSessionStore_Set(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		t.Run("Set new key", func(t *testing.T) {
			...
			// Verify it was set
			value, found, err := ss.Get(ctx, "new-key", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.True(t, found)
		...
		t.Run("Set existing key (upsert)", func(t *testing.T) {
			// Set initial value
			err := ss.Set(ctx, "existing-key", []byte("initial-value"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Update the value
			err = ss.Set(ctx, "existing-key", []byte("updated-value"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Verify it was updated
			value, found, err := ss.Get(ctx, "existing-key", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.True(t, found)
		...
		t.Run("Set without sync ID", func(t *testing.T) {
			err := ss.Set(ctx, "test-key", []byte("test-value"))
			require.Error(t, err)
			require.Contains(t, err.Error(), "sync id is required")
		})

		t.Run("Set empty value", func(t *testing.T) {
			err := ss.Set(ctx, "empty-key", []byte(""), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			value, found, err := ss.Get(ctx, "empty-key", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.True(t, found)
...
func TestSessionStore_GetMany(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		...
		t.Run("GetMany non-existent keys", func(t *testing.T) {
			result, unprocessedKeys, err := ss.GetMany(ctx, []string{"non-existent-1", "non-existent-2"}, sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.Empty(t, result)
			require.Empty(t, unprocessedKeys)
		})
		...
		t.Run("GetMany existing keys", func(t *testing.T) {
			// Set some values
			err := ss.Set(ctx, "key1", []byte("value1"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
			err = ss.Set(ctx, "key2", []byte("value2"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
			err = ss.Set(ctx, "key3", []byte("value3"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Get multiple keys
			result, unprocessedKeys, err := ss.GetMany(ctx, []string{"key1", "key2", "key3"}, sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.Empty(t, unprocessedKeys)
		...
		t.Run("GetMany mixed existing and non-existent keys", func(t *testing.T) {
			result, unprocessedKeys, err := ss.GetMany(ctx, []string{"key1", "non-existent", "key2"}, sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.Empty(t, unprocessedKeys)
		...
		t.Run("GetMany with prefix", func(t *testing.T) {
			...
			err := ss.Set(ctx, "prefixed-key1", []byte("prefixed-value1"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("test-prefix:"))
			require.NoError(t, err)
			err = ss.Set(ctx, "prefixed-key2", []byte("prefixed-value2"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("test-prefix:"))
			require.NoError(t, err)

			// Get multiple keys with prefix
			result, unprocessedKeys, err := ss.GetMany(ctx, []string{"prefixed-key1", "prefixed-key2"},
				sessions.WithSyncID(syncID), sessions.WithPrefix("test-prefix:"))
			require.NoError(t, err)
			require.Empty(t, unprocessedKeys)
			require.Len(t, result, 2)
		...
		t.Run("GetMany without sync ID", func(t *testing.T) {
			_, unprocessedKeys, err := ss.GetMany(ctx, []string{"key1"})
			require.Error(t, err)
			require.Contains(t, err.Error(), "sync id is required")
			require.Empty(t, unprocessedKeys)
		})
		...
		t.Run("GetMany size limit enforcement", func(t *testing.T) {
			...
			for i, key := range keys {
				...
				err := ss.Set(ctx, key, value, sessions.WithSyncID(syncID))
				require.NoError(t, err)
			}

			result, unprocessedKeys, err := ss.GetMany(ctx, keys, sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("GetMany mixed large and small values", func(t *testing.T) {
			...
			for _, key := range smallKeys {
				err := ss.Set(ctx, key, bytes.Repeat([]byte("small"), 20), sessions.WithSyncID(syncID))
				require.NoError(t, err)
			}
			...
			result, unprocessedKeys, err := ss.GetMany(ctx, allKeys, sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("GetMany unprocessedKeys for non-existent keys", func(t *testing.T) {
			...
			result, unprocessedKeys, err := ss.GetMany(ctx, nonExistentKeys, sessions.WithSyncID(syncID))
			require.NoError(t, err)
			...
			err = ss.Set(ctx, "mixed-exists", []byte("exists"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			mixedKeys := []string{"mixed-exists", "mixed-non-existent1", "mixed-non-existent2"}
			result, unprocessedKeys, err = ss.GetMany(ctx, mixedKeys, sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("GetMany unprocessedKeys ordering and completeness", func(t *testing.T) {
			...
			for i := range numKeys {
				...
				err := ss.Set(ctx, keys[i], value, sessions.WithSyncID(syncID))
				require.NoError(t, err)
			}
		...
		t.Run("GetMany empty values with size limit", func(t *testing.T) {
			...
			for _, key := range emptyKeys {
				err := ss.Set(ctx, key, []byte{}, sessions.WithSyncID(syncID))
				require.NoError(t, err)
			}
...
func TestSessionStore_SetMany(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		...
		t.Run("SetMany multiple keys", func(t *testing.T) {
			...
			err := ss.SetMany(ctx, values, sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Verify all values were set
			for key, expectedValue := range values {
				value, found, err := ss.Get(ctx, key, sessions.WithSyncID(syncID))
				require.NoError(t, err)
				require.True(t, found)
...
func TestSessionStore_Delete(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		...
		t.Run("Delete existing key", func(t *testing.T) {
			// Set a value first
			err := ss.Set(ctx, "to-delete", []byte("will-be-deleted"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Verify it exists
			value, found, err := ss.Get(ctx, "to-delete", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.True(t, found)
			require.Equal(t, []byte("will-be-deleted"), value)

			// Delete it
			err = ss.Delete(ctx, "to-delete", sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("Delete with prefix", func(t *testing.T) {
			// Set a value with prefix
			err := ss.Set(ctx, "prefixed-to-delete", []byte("will-be-deleted"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("delete-prefix:"))
			require.NoError(t, err)

			// Verify it exists
			value, found, err := ss.Get(ctx, "prefixed-to-delete",
				sessions.WithSyncID(syncID), sessions.WithPrefix("delete-prefix:"))
			require.NoError(t, err)
			require.True(t, found)
			require.Equal(t, []byte("will-be-deleted"), value)
			...
			err = ss.Delete(ctx, "prefixed-to-delete",
				sessions.WithSyncID(syncID), sessions.WithPrefix("delete-prefix:"))
			require.NoError(t, err)
...
func TestSessionStore_Clear(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		...
		t.Run("Clear all keys", func(t *testing.T) {
			// Set multiple values
			err := ss.Set(ctx, "clear-key1", []byte("clear-value1"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
			err = ss.Set(ctx, "clear-key2", []byte("clear-value2"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
			err = ss.Set(ctx, "clear-key3", []byte("clear-value3"), sessions.WithSyncID(syncID))
			require.NoError(t, err)

			// Verify they exist
			all, pageToken, err := ss.GetAll(ctx, "", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.Len(t, all, 3)
		...
		t.Run("Clear with prefix", func(t *testing.T) {
			...
			err := ss.Set(ctx, "prefix1-key1", []byte("prefix1-value1"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("prefix1:"))
			require.NoError(t, err)
			err = ss.Set(ctx, "prefix1-key2", []byte("prefix1-value2"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("prefix1:"))
			require.NoError(t, err)
			err = ss.Set(ctx, "prefix2-key1", []byte("prefix2-value1"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("prefix2:"))
			require.NoError(t, err)

			// Clear only prefix1
			err = ss.Clear(ctx, sessions.WithSyncID(syncID), sessions.WithPrefix("prefix1:"))
			require.NoError(t, err)

			// Verify prefix1 keys are gone but prefix2 remains
			_, found, err := ss.Get(ctx, "prefix1-key1",
				sessions.WithSyncID(syncID), sessions.WithPrefix("prefix1:"))
			require.NoError(t, err)
			require.False(t, found)

			err = ss.Clear(ctx, sessions.WithSyncID(syncID), sessions.WithPrefix("a%"))
			require.NoError(t, err)

			value, found, err := ss.Get(ctx, "prefix2-key1",
				sessions.WithSyncID(syncID), sessions.WithPrefix("prefix2:"))
			require.NoError(t, err)
			require.True(t, found)
		...
		t.Run("Clear with special characters in prefix (underscore, percent, backslash)", func(t *testing.T) {
			...
			require.NoError(t, ss.Set(ctx, "k1", []byte("v1"), sessions.WithSyncID(syncID), sessions.WithPrefix("a_b:")))
			require.NoError(t, ss.Set(ctx, "k1x", []byte("v1x"), sessions.WithSyncID(syncID), sessions.WithPrefix("aXb:"))) // control for "_"
			require.NoError(t, ss.Set(ctx, "k2", []byte("v2"), sessions.WithSyncID(syncID), sessions.WithPrefix("a%:")))
			require.NoError(t, ss.Set(ctx, "k3", []byte("v3"), sessions.WithSyncID(syncID), sessions.WithPrefix(`a\b:`)))
			require.NoError(t, ss.Set(ctx, "k4", []byte("v4"), sessions.WithSyncID(syncID), sessions.WithPrefix("control:")))

			// Clear underscore prefix only.
			require.NoError(t, ss.Clear(ctx, sessions.WithSyncID(syncID), sessions.WithPrefix("a_b:")))
			_, found, err := ss.Get(ctx, "k1", sessions.WithSyncID(syncID), sessions.WithPrefix("a_b:"))
			require.NoError(t, err)
			require.False(t, found)

			// "aXb:" should NOT be cleared (it would be if "_" were treated as a wildcard).
			_, found, err = ss.Get(ctx, "k1x", sessions.WithSyncID(syncID), sessions.WithPrefix("aXb:"))
			require.NoError(t, err)
			require.True(t, found, "aXb: should remain after clearing a_b:")

			// Other prefixes should remain.
			_, found, err = ss.Get(ctx, "k2", sessions.WithSyncID(syncID), sessions.WithPrefix("a%:"))
			require.NoError(t, err)
			require.True(t, found)
			_, found, err = ss.Get(ctx, "k3", sessions.WithSyncID(syncID), sessions.WithPrefix(`a\b:`))
			require.NoError(t, err)
			require.True(t, found)
			_, found, err = ss.Get(ctx, "k4", sessions.WithSyncID(syncID), sessions.WithPrefix("control:"))
			require.NoError(t, err)
			require.True(t, found)

			// Clear percent prefix only.
			require.NoError(t, ss.Clear(ctx, sessions.WithSyncID(syncID), sessions.WithPrefix("a%:")))
			_, found, err = ss.Get(ctx, "k2", sessions.WithSyncID(syncID), sessions.WithPrefix("a%:"))
			require.NoError(t, err)
			require.False(t, found)
			...
			require.NoError(t, ss.Clear(ctx, sessions.WithSyncID(syncID), sessions.WithPrefix(`a\b:`)))
			_, found, err = ss.Get(ctx, "k3", sessions.WithSyncID(syncID), sessions.WithPrefix(`a\b:`))
			require.NoError(t, err)
			require.False(t, found)
...
func TestSessionStore_GetAll(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		...
		t.Run("GetAll with multiple keys", func(t *testing.T) {
			// Set multiple values
			err := ss.Set(ctx, "getall-key1", []byte("getall-value1"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
			err = ss.Set(ctx, "getall-key2", []byte("getall-value2"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
			err = ss.Set(ctx, "getall-key3", []byte("getall-value3"), sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("GetAll with prefix", func(t *testing.T) {
			// Set values with different prefixes
			err := ss.Set(ctx, "getall-prefix1-key1", []byte("getall-prefix1-value1"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("getall-prefix1:"))
			require.NoError(t, err)
			err = ss.Set(ctx, "getall-prefix1-key2", []byte("getall-prefix1-value2"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("getall-prefix1:"))
			require.NoError(t, err)
			err = ss.Set(ctx, "getall-prefix2-key1", []byte("getall-prefix2-value1"),
				sessions.WithSyncID(syncID), sessions.WithPrefix("getall-prefix2:"))
			require.NoError(t, err)

			// Get all with prefix1
			result, pageToken, err := ss.GetAll(ctx, "", sessions.WithSyncID(syncID), sessions.WithPrefix("getall-prefix1:"))
			require.NoError(t, err)
			require.Len(t, result, 2)
		...
		t.Run("GetAll with percent in prefix", func(t *testing.T) {
			...
			got, pageToken, err := ss.GetAll(ctx, "", sessions.WithSyncID(syncID), sessions.WithPrefix("a%:"))
			require.NoError(t, err)
			require.Equal(t, "", pageToken)
			require.Len(t, got, 1)
			require.Equal(t, []byte("v1"), got["k1"])
		...
		t.Run("GetAll with backslash in prefix", func(t *testing.T) {
			...
			got, pageToken, err := ss.GetAll(ctx, "", sessions.WithSyncID(syncID), sessions.WithPrefix(`a\b:`))
			require.NoError(t, err)
			require.Equal(t, "", pageToken)
			require.Len(t, got, 1)
			require.Equal(t, []byte("v1"), got["k1"])
		...
		t.Run("GetAll pagination with size limit enforcement", func(t *testing.T) {
			...
			iteration := 0
			for {
				result, nextPageToken, err := ss.GetAll(ctx, pageToken, sessions.WithSyncID(syncID))
				require.NoError(t, err)
				pageCount++
				iteration++
		...
		t.Run("GetAll pageToken continuation", func(t *testing.T) {
			...
			allItems, _, err := ss.GetAll(ctx, "", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			...
			pageFromMiddle, pageToken, err := ss.GetAll(ctx, startKey, sessions.WithSyncID(syncID))
			require.NoError(t, err)
		...
		t.Run("GetAll exactly 100 items", func(t *testing.T) {
			...
			if pageToken != "" {
				...
				result2, pageToken2, err := ss.GetAll(ctx, pageToken, sessions.WithSyncID(syncID))
				require.NoError(t, err)
		...
		t.Run("GetAll pageToken value correctness", func(t *testing.T) {
			...
			// Get first page
			page1, pageToken1, err := ss.GetAll(ctx, "", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			...
			if pageToken1 != "" {
				...
				page2, _, err := ss.GetAll(ctx, pageToken1, sessions.WithSyncID(syncID))
				require.NoError(t, err)
		...
		t.Run("GetAll pagination with prefix multiple pages", func(t *testing.T) {
			...
			for i := range 150 {
				...
				value := []byte(fmt.Sprintf("value-%d", i))
				err := ss.Set(ctx, key, value, sessions.WithSyncID(syncID), sessions.WithPrefix("prefix-pag:"))
				require.NoError(t, err)
			}
			...
			prevAllLen := 0
			for {
				result, nextPageToken, err := ss.GetAll(ctx, pageToken, sessions.WithSyncID(syncID), sessions.WithPrefix("prefix-pag:"))
				require.NoError(t, err)
				iteration++
				maps.Copy(all, result)
		...
		t.Run("GetAll pageToken with prefix continuation", func(t *testing.T) {
			...
			for i := range 150 {
				...
				value := []byte(fmt.Sprintf("value-%d", i))
				err := ss.Set(ctx, key, value, sessions.WithSyncID(syncID), sessions.WithPrefix("prefix-cont:"))
				require.NoError(t, err)
			}
			...
			prevAllLen := 0
			for {
				result, nextPageToken, err := ss.GetAll(ctx, pageToken, sessions.WithSyncID(syncID), sessions.WithPrefix("prefix-cont:"))
				require.NoError(t, err)
				iteration++
				maps.Copy(all, result)
...
func TestSessionStore_Isolation(t *testing.T) {
	...
	for _, engine := range []c1zstore.Engine{c1zstore.EngineSQLite, c1zstore.EnginePebble} {
		t.Run(string(engine), func(t *testing.T) {
			...
			syncID1, err := store.StartNewSync(ctx, "full", "")
			require.NoError(t, err)
			...
			syncID2, err := store.StartNewSync(ctx, "full", "")
			require.NoError(t, err)

			t.Run("Sync ID isolation", func(t *testing.T) {
				// Set values in different sync IDs
				err := ss.Set(ctx, "isolated-key", []byte("sync1-value"), sessions.WithSyncID(syncID1))
				require.NoError(t, err)
				err = ss.Set(ctx, "isolated-key", []byte("sync2-value"), sessions.WithSyncID(syncID2))
				require.NoError(t, err)

				// Verify isolation
				value1, found1, err := ss.Get(ctx, "isolated-key", sessions.WithSyncID(syncID1))
				require.NoError(t, err)
				require.True(t, found1)
				require.Equal(t, []byte("sync1-value"), value1)

				value2, found2, err := ss.Get(ctx, "isolated-key", sessions.WithSyncID(syncID2))
				require.NoError(t, err)
			...
			t.Run("Concurrent reads and writes", func(t *testing.T) {
				...
				for i := 0; i < 5; i++ {
					go func(i int) {
						...
						errCh <- ss.Set(ctx, key, value, sessions.WithSyncID(syncID))
					}(i)
				}
				...
				for i := 0; i < 5; i++ {
					go func(i int) {
						defer func() { done <- true }()
						key := fmt.Sprintf("concurrent-key-%d", i)
						_, _, err := ss.Get(ctx, key, sessions.WithSyncID(syncID))
						errCh <- err
					}(i)
				}
...
func TestSessionStore_ErrorHandling(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		t.Run("Invalid options", func(t *testing.T) {
			...
			_, _, err := ss.Get(ctx, "test-key", invalidOption)
			require.Error(t, err)
		...
		t.Run("Context cancellation", func(t *testing.T) {
			...
			_, _, err := ss.Get(cancelCtx, "test-key", sessions.WithSyncID(syncID))
			require.Error(t, err)
...
func TestSessionStore_Performance(t *testing.T) {
	forEachSessionStoreEngine(t, func(t *testing.T, ctx context.Context, store c1zstore.Store, syncID string, ss sessions.SessionStore) {
		t.Run("Large batch operations", func(t *testing.T) {
			...
			err := ss.SetMany(ctx, largeValues, sessions.WithSyncID(syncID))
			require.NoError(t, err)
			...
			pageToken := ""
			for {
				items, nextPageToken, err := ss.GetAll(ctx, pageToken, sessions.WithSyncID(syncID))
				require.NoError(t, err)
		...
		t.Run("Large values", func(t *testing.T) {
			...
			err := ss.Set(ctx, "large-value-key", largeValue, sessions.WithSyncID(syncID))
			require.NoError(t, err)

			value, found, err := ss.Get(ctx, "large-value-key", sessions.WithSyncID(syncID))
			require.NoError(t, err)
			require.True(t, found)
