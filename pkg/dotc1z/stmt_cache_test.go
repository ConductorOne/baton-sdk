...
func TestQueryStats_DisabledByDefault(t *testing.T) {
	ctx := context.Background()
	db, cleanup := setupTestDB(t)
	defer cleanup()
	...
	// Get query stats - should be empty if disabled (default)
	queryStats := GetQueryStats(db)
	// If stats are disabled, GetQueryStats returns empty slice
	if queryStats == nil {
		t.Error("GetQueryStats() should not return nil")
	}
	// When disabled, it should return empty slice (not nil)
	if len(queryStats) > 0 && !queryStatsEnabled {
		t.Error("GetQueryStats() should return empty slice when stats are disabled")
	}
}

func TestQueryStats_CanBeEnabled(t *testing.T) {
	ctx := context.Background()
	db, cleanup := setupTestDB(t)
	defer cleanup()
