...
func TestC1Z(t *testing.T) {
	...
	var opts []V1C1ZOption
	opts = append(opts, WithPragma("journal_mode", "WAL"))

	// Open file
	f, err := NewV1C1ZFile(ctx, testFilePath, opts...)
...
func TestC1ZDecoder(t *testing.T) {
	...
	// Open file
	f, err := NewV1C1ZFile(ctx, testFilePath, WithPragma("journal_mode", "WAL"))
	require.NoError(t, err)
	...
	dbFile, err := os.ReadFile(filepath.Join(f.workingDir, "db"))
	require.NoError(t, err)
...
func TestC1ZInvalidFile(t *testing.T) {
	...
	_, err = NewV1C1ZFile(ctx, testFilePath, WithPragma("journal_mode", "WAL"))
