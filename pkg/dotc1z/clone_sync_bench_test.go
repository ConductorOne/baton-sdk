package dotc1z

import (
	"context"
	"fmt"
	"os"
	"path/filepath"
	"runtime"
	"testing"

	"github.com/stretchr/testify/require"
)

func cloneSyncBenchTestdataPath(syncID string) string {
	_, filename, _, _ := runtime.Caller(0)
	return filepath.Join(filepath.Dir(filename), "..", "sync", "expand", "testdata", fmt.Sprintf("sync.%s.expanded", syncID))
}

func benchmarkCloneSync(b *testing.B, syncID string) {
	c1zPath := cloneSyncBenchTestdataPath(syncID)
	info, err := os.Stat(c1zPath)
	if os.IsNotExist(err) {
		b.Skipf("testdata file not found: %s", c1zPath)
	}
	require.NoError(b, err)

	// Do not use b.Context() here. Doing so causes the benchmark to run slower.
	// The SQL library's interruptOnDone() is called if ctx.Done() is not nil.
	ctx := context.Background()

	tempDir := b.TempDir()
	sourcePath := filepath.Join(tempDir, fmt.Sprintf("source.%s.c1z", syncID))
	srcData, err := os.ReadFile(c1zPath)
	require.NoError(b, err)
	//nolint:gosec // sourcePath is created under b.TempDir in this benchmark.
	err = os.WriteFile(sourcePath, srcData, 0600)
	require.NoError(b, err)

	c1f, err := NewC1ZFile(ctx, sourcePath, WithTmpDir(tempDir))
	require.NoError(b, err)
	defer c1f.Close(ctx)

	err = c1f.SetSyncID(ctx, syncID)
	require.NoError(b, err)
	err = c1f.EndSync(ctx)
	require.NoError(b, err)

	b.SetBytes(info.Size())
	b.ReportAllocs()
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		outPath := filepath.Join(tempDir, fmt.Sprintf("clone-%d.c1z", i))

		b.StartTimer()
		err = c1f.CloneSync(ctx, outPath, syncID)
		b.StopTimer()
		require.NoError(b, err)

		require.NoError(b, os.Remove(outPath))
	}

	err = c1f.Close(ctx)
	require.NoError(b, err)
}

func BenchmarkCloneSyncSmall(b *testing.B) {
	benchmarkCloneSync(b, "36zGvJw3uxU1QMJKU2yPVQ1hBOC")
}

func BenchmarkCloneSyncSmallMedium(b *testing.B) {
	benchmarkCloneSync(b, "36zM46KKuaBq0wjSSvKh5o0350y")
}
