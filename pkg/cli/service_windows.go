...
func initLogger(ctx context.Context, name string, loggingOpts ...logging.Option) (context.Context, error) {
	if isService() {
		// On Windows, stdout/stderr on services goes nowhere. Hook up to the event log.
		...
		// The event log is closed by runService.
...
func runService(ctx context.Context, name string) (context.Context, error) {
	...
	if !ok {
		l.Error("Event log not found in context. Opening new event log.")
		var err error
		elog, err = eventlog.Open(name)
		if err != nil {
