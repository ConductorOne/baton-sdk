...
func Generate(name string, schema field.Configuration) {
	...
	// Sync + Close + err-check so a write-back failure surfaces as
	// a panic rather than producing a silently-torn conf.gen.go.
	// The deferred Close stays as a safety net for the panic paths
	// above; on the success path fClosed=true suppresses it.
