...
func Generate[T connectorbuilder.ConnectorBuilder](name string, schema field.Configuration) {
	if name == "" {
	...
	type StructInfo struct {
		...
		DefaultBuilderPkgPath string
	}

	var t T
	builderType := reflect.TypeOf(t)

	if builderType.Kind() == reflect.Ptr {
		builderType = builderType.Elem()
	}

	data := StructInfo{
		StructName:            toPascalCase(name),
		Fields:                []FieldInfo{},
		DefaultBuilder:        builderType.String(),
		DefaultBuilderPkgPath: builderType.PkgPath(),
	}
...
func (c* {{ .StructName }}) DefaultBuilder() any {
	var t {{ .DefaultBuilder }}
	return &t
}
