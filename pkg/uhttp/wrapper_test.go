...
// regression where a header opted into the cache key via WithCacheKeyHeaders
// gets mutated by a RoundTripper further down the transport chain
// (transport.go's userAgentTripper does exactly this for User-Agent, which
// is why this used to bite by default before the cache key was scoped down
...
func TestWrapper_Do_CachesDespiteRoundTripHeaderInjection(t *testing.T) {
	...
	client, err := NewBaseHttpClientWithContext(ctx, httpClient, WithCacheKeyHeaders("X-Injected-By-Transport"))
	require.NoError(t, err)
