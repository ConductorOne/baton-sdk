...
// guards against a regression where a header opted into the cache key via
...
// User-Agent). BaseHttpClient.do computes the cache key from req before the
// round trip (Get) and again from the same req after it (Set). http.Client.Do
// forks the *http.Request struct on every call once Timeout > 0 (which
// uhttp.NewClient always sets), but that fork is shallow, so Header stays the
// same map the caller passed in -- any RoundTripper that mutates it mutates
// the very request do() is holding. If do() round-tripped on req itself
// rather than a clone, the Set key would differ from every future Get key for
// that header and the cache would become write-only.
func TestWrapper_DoWithCacheKeyHeaders_CachesDespiteRoundTripHeaderInjection(t *testing.T) {
	var hits int32
	ts := newCountingServer(&hits)
	defer ts.Close()
