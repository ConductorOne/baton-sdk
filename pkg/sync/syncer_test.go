...
func TestExpandGrantImmutableCycle(t *testing.T) {
	runWithSyncModes(t, func(t *testing.T, extraOpts []SyncOpt) {
		...
		entReq := reader_v2.GrantsReaderServiceListGrantsForEntitlementRequest_builder{
			Entitlement: group1Ent,
			PageToken:   "",
			Annotations: nil,
		}.Build()
		resp, err := store.ListGrantsForEntitlement(ctx, entReq)
		require.NoError(t, err)
		...
		principalReq2 := reader_v2.GrantsReaderServiceListGrantsForPrincipalRequest_builder{
			PrincipalId: user2.GetId(),
			PageToken:   "",
			Annotations: nil,
		}.Build()
		principalResp2, err := store.ListGrantsForPrincipal(ctx, principalReq2)
		require.NoError(t, err)
		require.Len(t, principalResp2.GetList(), 1)

		grant = principalResp2.GetList()[0]
