...
func TestPartialSync(t *testing.T) {
	runWithSyncModes(t, func(t *testing.T, extraOpts []SyncOpt) {
		...
		partialSyncer, err := NewSyncer(ctx, mc, opts...)
		require.NoError(t, err)
...
func TestExternalResourceMatchAllBatchedFlush(t *testing.T) {
	runWithSyncModes(t, func(t *testing.T, extraOpts []SyncOpt) {
		...
		// that records every PutGrants call. Pebble specifically, so the
		// placeholder's deletion below actually exercises minimalGrantForDelete's
		// refs-based path (DeleteGrantByRefs) rather than SQLite's id fallback,
		// which doesn't implement grantByRefsDeleter at all.
		internalC1zpath := filepath.Join(tempDir, "internal.c1z")
		rawStore, err := dotc1z.NewStore(ctx, internalC1zpath, dotc1z.WithEngine(c1zstore.EnginePebble), dotc1z.WithTmpDir(tempDir))
		require.NoError(t, err)
		...
		store, err := dotc1z.NewStore(ctx, internalC1zpath, dotc1z.WithEngine(c1zstore.EnginePebble), dotc1z.WithTmpDir(tempDir))
		require.NoError(t, err)
		grants, err := store.ListGrantsForEntitlement(ctx, reader_v2.GrantsReaderServiceListGrantsForEntitlementRequest_builder{
			Entitlement: internalGroupEnt,
		}.Build())
...
func TestExternalResourceWithGrantToEntitlement(t *testing.T) {
	runWithSyncModes(t, func(t *testing.T, extraOpts []SyncOpt) {
		...
		require.NoError(t, err)
		require.Len(t, resp.GetSyncs(), 1, "Should have one sync")
		...
		require.NoError(t, err)
		require.Len(t, clonedSyncs.GetSyncs(), 1, "Should have one sync")
		clonedSyncID := clonedSyncs.GetSyncs()[0].GetId()
		require.Equal(t, syncID, clonedSyncID, "Sync IDs should be identical")
		err = clonedStore.Close(ctx)
		require.NoError(t, err)
	})
}
...
// TestExclusionGroupTracker exercises the pure validation core behind
// ingestion invariant I5 (ingest_invariants.go): one resource type per
// group, one default per group, size cap. It replaced the page-level
// streaming validator, so the error-text assertions here pin the
// messages connectors and operators already know.
func TestExclusionGroupTracker(t *testing.T) {
	...
	record := func(tr *exclusionGroupTracker, ents ...*v2.Entitlement) error {
		for _, ent := range ents {
			if err := tr.record(ent); err != nil {
				return err
			}
		}
		return nil
	...
	t.Run("re-recording the same default is idempotent", func(t *testing.T) {
		tr := &exclusionGroupTracker{}
		viewer := newEntitlement(t, userResourceType, "user1", "viewer", et.WithExclusionGroupDefault("role", 1))
		require.NoError(t, record(tr, viewer))
		require.NoError(t, tr.record(viewer))
	})
	...
	t.Run("group at the cap passes", func(t *testing.T) {
		...
		for i := 0; i < maxEntitlementsPerExclusionGroup; i++ {
			require.NoError(t, tr.record(newEntitlement(t, userResourceType, fmt.Sprintf("user%d", i), "member", et.WithExclusionGroup("role"))))
		}
	})
	...
	t.Run("group over the cap fails", func(t *testing.T) {
		...
		for i := 0; i < maxEntitlementsPerExclusionGroup+1; i++ {
			err = tr.record(newEntitlement(t, userResourceType, fmt.Sprintf("user%d", i), "member", et.WithExclusionGroup("role")))
			if err != nil {
				break
			}
		}
