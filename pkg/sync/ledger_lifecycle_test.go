...
func TestLedgerFinishedProcessingResumesWithoutReset(t *testing.T) {
	for _, cut := range []string{"after-clear", "after-page"} {
		t.Run(cut, func(t *testing.T) {
			...
			ready, err := s.prepareLedgerState(t.Context(), "processing", false)
			require.NoError(t, err)
			...
			ready, err = s.prepareLedgerState(t.Context(), "resumed", false)
...
func TestLedgerSealReadyUnfinishedDoesNotStartAnotherPass(t *testing.T) {
	...
	f.audit.enter(ledgerWalk)
	ready, err := s.prepareLedgerState(t.Context(), "resume", false)
	f.audit.enter(ledgerLifecycle)
	require.NoError(t, err)
