...
func TestPebble_EtagReplay_InsertResourceGrants_PreservesResource(t *testing.T) {
	...
	tempDir := t.TempDir()
	sync1Path := filepath.Join(tempDir, "etag-irg-prev.c1z")
	sync2Path := filepath.Join(tempDir, "etag-irg-current.c1z")
	...
	// ETagMatch + InsertResourceGrants ---
	mc.mu.Lock()
	...
	// --- Inspect the group in the resulting c1z (file B) ---
