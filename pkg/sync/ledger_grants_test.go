...
func TestLedgerGrantCommitFailureRetry(t *testing.T) {
	...
	f.audit.enter(ledgerHandler)
	require.ErrorIs(t, s.invokeActionPage(t.Context(), s.run.current(), s.SyncGrants, false), errLedgerInjectedPage)
	f.audit.enter(ledgerLifecycle)
...
func TestLedgerGrantPlanner(t *testing.T) {
	...
	s.ledger.store = ledgerFailingPageStore{PageLedgerStore: f.ledger, stage: "commit"}
	f.audit.enter(ledgerHandler)
	require.ErrorIs(t, s.invokeActionPage(t.Context(), action, s.SyncGrants, false), errLedgerInjectedPage)
	f.audit.enter(ledgerLifecycle)
	require.False(t, s.run.getAction(action.ID).TypeScopedPlanned)
	s.ledger.store = f.ledger
	f.audit.enter(ledgerHandler)
	require.NoError(t, s.invokeActionPage(t.Context(), action, s.SyncGrants, false))
	f.audit.enter(ledgerLifecycle)
	continued := s.run.getAction(action.ID)
	require.True(t, continued.TypeScopedPlanned)
	f.audit.enter(ledgerHandler)
	require.NoError(t, s.invokeActionPage(t.Context(), continued, s.SyncGrants, false))
	f.audit.enter(ledgerLifecycle)
