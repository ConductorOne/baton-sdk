...
func TestWhaleInvariantCost(t *testing.T) {
	...
	facts, ok := store.(dotc1z.IngestFactStore)
	require.True(t, ok)
	sc, isSC := store.(dotc1z.SourceCacheStore)
	...
	// validateStoredExclusionGroups performs it (full entitlement listing
	...
	// --- I6: scope-index vs manifest consistency.
	if isSC {
		start = time.Now()
