...
// Regression test: after a full expansion pass on a sync, needs_expansion should be cleared
// so a later incremental run on the same sync starts from a clean slate.
func TestFullExpansion_ClearsNeedsExpansionFlags(t *testing.T) {
	ctx := context.Background()
	tmpDir := t.TempDir()
	...
	c1f, syncID := setupExpandableSync(ctx, t, tmpDir, "full_expansion_needs_flags",
		[]*v2.ResourceType{groupRT, userRT},
		[]*v2.Resource{g1, g2, u1},
		[]*v2.Entitlement{e1, e2},
		[]*v2.Grant{grantU1E1, grantG1E2},
	)
	defer c1f.Close(ctx)

	// Run a full expansion pass for this sync.
	require.NoError(t, runFullExpansion(ctx, c1f, syncID))

	// After full expansion, there should be no remaining needs_expansion rows.
	// If this fails, the first subsequent incremental run on this sync will over-expand.
	resp, err := c1f.ListGrantsInternal(ctx, connectorstore.GrantListOptions{
		Mode:               connectorstore.GrantListModeExpansionNeedsOnly,
		SyncID:             syncID,
		NeedsExpansionOnly: true,
