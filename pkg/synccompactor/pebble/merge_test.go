...
func TestMergeIntoSkipsGrantEntitlementIDsWhenDisabled(t *testing.T) {
	ctx := context.Background()
	now := time.Unix(2000, 0).UTC()
	dest, destSync := seedBase(t, ctx, "entids-disabled-dest", recordSet{})
	src := buildEngineSource(t, ctx, "entids-disabled-src", recordSet{
	...
	require.Nil(t, stats.GrantEntitlementIDs,
		"flag-off merge must not allocate the changed-entitlement hashmap")
}
