...
| Fresh sync without surviving Init | CO-024 distinguishes an empty unfinished binding through BoundSyncUnstarted. Its bounded read-only query rejects checkpoint/archive/record/replay history and errors; only an empty start initializes clean quality. Init persists that fact. The public 40-case differential now requires exact quality equality; legacy/finished/blocked-state guards retain their behavior. |
