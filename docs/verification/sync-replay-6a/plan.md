...
- PR placement: originally a separate stacked follow-up; landed in this PR
  per CO-014.
- Status: landed. Seal mechanics are specified by CO-014; the closure-gate
  instruments and the fixed-total-row scope-scaling benchmark are recorded in
  the CO-014/CO-015/CO-016 remediation evidence section of `evidence.md`.
- Compatibility: none required. Source-cache manifests have never shipped, so
  every replay-eligible artifact is written by counting code. There is no
  legacy-manifest population and no fallback mode.
- Action: add a primary-derived `row_count` to every sealed
  `(row_kind, scope_key)` manifest entry, with wire-level presence retained so
  absence stays distinguishable from an explicit zero. Fuse count collection
  into EndSync's existing primary scans where available, cover
  deferred/stashed scan paths, clear counts before a completed sync is rebound
  for mutation, and recompute them when a rebound sync reseals.
- Replay contract: preflight retains scoped index→primary/stamp validation and
  requires its cardinality to equal the sealed count before destination
  mutation. A replay-eligible manifest entry without a count is a hard
  preflight error (seal invariant violation), not a fallback trigger; the full
  primary↔index scan is deleted outright. Count zero means a proven empty
  scope, not unknown.
- Verification delta: sealed manifest row counts on every manifest write,
  missing-count hard rejection, rebind invalidation with reseal recount,
  counted corruption checks, and many-scope benchmarks.
- Closure gate: all three row kinds prove primary-derived count reconciliation,
  ...
  rejection plus missing-count rejection, failure/retry without a false count,
  read-only reopen/clone preservation, and stale-count removal on rebind. A
  fixed-total-row benchmark varying the number of replayed scopes must
  demonstrate preflight cost no longer scales with the row kind's total
  primary count (the O(S·N) scan is gone); the independent biconditional
  auditor remains count-unaware.
...
- Production delta: fold marks its output compacted and range-deletes only source
  ...
  introspection without payload unpack or zstd decode. Expanded-grant overwrite
  loops now put each successful `Get` in a per-record function with an immediate
  deferred close (the grant IfNewer loop got the same fix before diff-sync
  removal deleted that path). Source-scope maintenance adds a point `Get`
...
### CO-014 — CO-004 integration and seal-count mechanics

- Type: placement change plus implementation-mechanics record for CO-004.
- Source: review of replay orchestration obligations concluded the O(S·N)
  preflight cannot ship even transitionally — a many-scope replay against a
  large row kind is a production stall, not a degradation. CO-004 therefore
  lands in this PR rather than a stacked follow-up.
- Production delta: `SourceCacheEntryRecord` gains `optional uint64 row_count`
  (wire presence retained; zero means proven-empty, absence means not sealed
  by a counting EndSync). Counting is a separate seal pass in EndSync's
  finalize sequence, not fused into the deferred-index scan: it runs sealed,
  after the deferred index build, strictly BEFORE the `ended_at` stamp, so no
  artifact can carry the finished verdict without the counts (they ride the
  WAL the stamp fsyncs; a crash between leaves the sync unfinished and the
  resumed EndSync recounts). One primary-family scan per row kind that has
  manifest entries, counting value stamps via the raw scope scanner; cost is
  zero for syncs with an empty manifest. Counts derive from PRIMARIES, never
  the scope index, so the index cannot vouch for itself. `SetCurrentSync`
  clears counts when rebinding over a non-empty manifest: published artifacts
  always recount at reseal, and clearing keeps any unpublished rebound store
  fail-closed for every current and future reader.
- Replay contract: as CO-004 specified. Preflight walks the scope index,
  validates each entry resolves to a primary stamped with the scope, and
  requires walk cardinality to equal the sealed count; the manifest entry
  must exist, be non-invalidated, and carry a count, each on pain of hard
  error; the full primary↔index scan is deleted.
- Explicit non-claim: the sealed count vouches only for post-seal mechanical
  integrity. Mid-sync partition damage that precedes the seal leaves index,
  stamps, and count self-consistent with the damaged set and is invisible to
  cardinality checks. That obligation belongs to CO-015.

### CO-015 — Row-partition invariant and per-scope poison

- Type: contract extension (new durable mechanism).
- Source: pressure-testing replay orchestration against interleaved
  multi-scope emission and live delete paths found sync-time mutations that
  silently shrink a scope's stamped row set: a row restamped from one scope to
  another (or to unscoped) within a sync, and deletes of stamped rows by
  actors outside the scope — including the external-principal reconciliation
  path that runs in production today (CO-016). Replaying such a scope would
  silently drop rows; no post-hoc validation can detect it (see CO-014's
  non-claim).
- Contract delta: rows are partitioned one-scope-per-row per sync. Any staged
  mutation acting for scope S that removes a row from scope X's stamped set,
  where X differs from S and X is non-empty, atomically stages a durable
  poison marker for `(row_kind, X)` in the same batch as the mutation:
  - put side — restamps, including unscoped overwrites that clear a stamp;
  - delete side — record batches carry an acting scope, defaulting to
    unscoped, so maintenance deletes conservatively poison every stamped row
    they remove. Scoped tombstone paths and replay destination clearing act
    as their own scope and never self-poison.
  Ordering independence: a scope whose final set is damaged always loses its
  last write of the damaged row to a later cross-scope event, so it is
  poisoned under any interleaving, any per-row order, and any scope count;
  engine writes are serialized, so no event is unobserved. Poisoned scopes
  are lookup misses and hard preflight rejections; both staging and refusal
  are logged with scope, kind, and cause so a perpetually-cold scope is
  diagnosable, not mysterious (the staging warning is deduplicated per
  `(row_kind, scope_key)` per open at the engine's observer — marker
  staging dedups only per batch, and batches re-mint per chunk, so a
  persistently mis-partitioned connector would otherwise warn once per
  chunk). Poison keys live inside the source-cache keyspace bounds, so
  CO-013's compaction invalidation range-deletes them with the manifests.
- Production delta: the canonical-ID tombstone path gains the same
  bounded-commit chunking as the scoped tombstone paths (completing CO-003's
  bound across all tombstone forms) and carries the acting scope.
- Recorded exemption (trusted-caller fast paths): the expander's
  synthesized-grant writes (`PutSynthesizedGrantRecords`, the synth-layer
  ingest, and `putSynthesizedGrantContributionsBatch`) stage rows with no
  prior-value read under the caller's brand-new-identity guarantee — the
  topological merge's base stream reads every existing grant of the
  destination entitlement (`ListGrantsForEntitlement`), so a colliding
  identity routes to the read-before-write update path instead. A violated
  guarantee cannot stage poison (there is no observed prior value) but
  cannot be silent either: the loser's stale scope index resolves to a
  mis-stamped primary and the counted preflight hard-refuses before any
  destination mutation.
- Verification boundary: the randomized store model does not generate
  cross-scope events (its row picks stay within the acting scope), so
  CO-015 regressions are owned entirely by the directed poison matrix,
  including its interleaving orders. Extending the model with restamps and
  cross-scope deletes is named follow-up work, not assumed coverage.
- Verification delta: interleaved and out-of-order restamp property coverage
  (mixed orders poison exactly the damaged scopes, single-order overlap
  poisons only the loser); delete-side poison for unscoped maintenance
  deletes and cross-scope canonical-ID tombstones; self-scope exemption for
  scoped tombstones and replay clearing; expander rewrites preserve stamps;
  poison survives reopen and is dropped by compaction invalidation; replay
  and lookup rejection of poisoned scopes.

### CO-016 — Destructive external-principal reconciliation boundary

- Type: recorded limitation with named follow-up; no Phase 6a contract change.
- Source: external-principal reconciliation (`WithExternalResourceC1ZPath`,
  the Entra-directory pairing) deletes every match-annotated placeholder
  grant each sync — matched or not — and re-issues derived grants unscoped.
  Scopes emitting placeholder grants therefore lose stamped rows every sync.
- Finding: this is not a poison-tuning problem. Replay cannot compose with
  destructive reconciliation, because the sealed artifact no longer contains
  what the scope's fetch produced (connector truth) — only derived output
  computed against a directory snapshot that changes independently of the
  scope's upstream. Any replay of that baked output would freeze directory
  changes out of the scope's grants. CO-015's poison correctly marks these
  scopes; they cold-fetch every sync, which is byte-identical behavior to a
  pre-replay-feature sync plus bounded bookkeeping overhead.
- Blast radius: only connectors using `ExternalResourceMatch*` annotations
  (profile/email matching), and only their placeholder-emitting grant scopes.
  Directory syncs themselves (Entra-on-Entra delta caching) never run
  reconciliation and are unaffected; direct-object-ID grant references never
  meet reconciliation; resource and entitlement scopes are untouched.
- Named follow-up (out of Phase 6a scope): rework reconciliation as a
  non-destructive derived-state pass, mirroring grant expansion — placeholder
  grants persist stamped (replayable connector truth), matched grants are
  recomputed each sync against the current directory, read/export paths
  filter placeholders (self-identified by their match annotations; derived
  grants must not inherit them). That rework — a separate PR — is what makes
  grant-scope caching real for match-annotation connectors; nothing in this
  PR's mechanism changes when it lands, because unpoisoned scopes simply
  start replaying.
- Verification delta in this PR: reconciliation-shaped unscoped deletes of
  stamped placeholder rows must poison the emitting scope and refuse its
  replay, with the poison-staging telemetry asserted.

### CO-017 — Cross-version fold fence (pre-orchestration obligation)

- Type: recorded latent risk with a named pre-wiring obligation; unreachable
  in this PR (surfaced by the land-safety compat review).
- Source: this PR deliberately does not bump the `pebble3` manifest engine
  name (all keyspace additions are new byte families; released v0.25.0
  already reads and compacts `pebble3` artifacts). An OLDER pebble3 SDK's
  fold starts from a byte copy of the base (`copyFileForFold`), so a
  branch-written SCOPED artifact folded by an old binary keeps its
  source-cache manifest (sealed row counts), poison markers, and
  `by_source_scope` indexes while the old fold rewrites primaries, sets no
  `compacted` stamp, and invalidates nothing. The output is a finished FULL
  sync that passes `UsableAsReplaySource` and `validateReplaySourceEligible`,
  and the CO-014 count preflight cannot object because the old fold
  disturbed neither the indexes nor the manifest — replay against it would
  copy stale rows silently. Old-SDK k-way/overlay rebuilds are safe by
  construction: they materialize a fresh store from known families only, so
  the manifest is dropped and replay refuses loudly.
- Preconditions (none exist today): a connector actually writing scope
  stamps, replay orchestration actually consuming them (the next phase), AND
  an older pebble3 SDK binary compacting artifacts in the same pipeline.
- Named obligation, owned by the replay-orchestration phase and REQUIRED
  before any production connector opts in: fence old-fold outputs out of
  eligibility — candidates include consulting `SourceCacheCompatRecord`'s
  `sdk_materialization_generation` with a fold-disturbance witness the old
  fold cannot forge, a fold-generation stamp the eligibility gate requires
  to match the sync lifecycle record, or (bluntest) a manifest engine-name
  bump for scoped artifacts. A byte-copied compat record alone is NOT a
  fence — the old fold copies it intact.
...
5. **Malformed-value deletes (rawdb)** — a malformed old value makes its
   value-derived index obligations unknowable, so the delete fails closed rather
   than dropping the primary and stranding an index entry that points at it.
   `TestVerificationMalformedAllKindDeleteFailsClosed` requires the error and
   requires primary, by_source_scope, and by_parent to be undisturbed.
