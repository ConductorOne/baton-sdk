...
| C7 | `TestWriteMuHolders` passes on the tree. Planted violations, each detected and restored: a bare `writeMu.Lock()` in a new method (rule 4, fatal, masks the rest so planted separately); `e.db.MetaSet` in `IsSealed` (rule 1); `e.clearCurrentSyncLocked()` in `IsSealed` (rule 3); `e.seal()` inside `EndFreshSync`'s `withWriteAllowSealed` closure (rule 2); after review, `go func() { e.db.FlushMemtables() }()` and a field-assigned closure calling `e.db.WALSyncPoint()` placed in `Close` after its `Lock()` (rule 1; an earlier version marked every call after a direct `Lock()` as held, overriding the escape rules, so both would have passed). The tree needed no allowlist beyond `merge_surface.go`: the 19 first-pass findings resolved to local closures invoked synchronously (`flush := func…`), Open-time functions (reached only from `Open`), and helpers every caller of which holds the lock — all three are now computed, not listed |
