...
func (m *LookupRequest) validate(all bool) error {
	...
	if !_LookupRequest_ScopeHash_Pattern.MatchString(m.GetScopeHash()) {
		err := LookupRequestValidationError{
			field:  "ScopeHash",
...
var _LookupRequest_ScopeHash_Pattern = regexp.MustCompile("^[a-f0-9]{64}$")
...
func (m *LookupResponse) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetSourceCacheKey()) > 1024 {
		err := LookupResponseValidationError{
			field:  "SourceCacheKey",
	...
	if utf8.RuneCountInString(m.GetSourceEtag()) > 1024 {
		err := LookupResponseValidationError{
			field:  "SourceEtag",
			reason: "value length must be at most 1024 runes",
		}
